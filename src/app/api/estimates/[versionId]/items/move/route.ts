...
import { moveEstimateItemSchema } from "@/lib/estimates/schemas";
...
export async function POST(
  ...
  try {
    ...
    const body = moveEstimateItemSchema.parse(await parseJsonBody(request));
    const data = await moveEstimateItem(versionId, body);
    return ok(data);
  } catch (error) {
