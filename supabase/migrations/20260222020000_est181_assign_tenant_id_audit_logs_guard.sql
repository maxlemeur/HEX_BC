-- Fix shared tenant trigger function to avoid direct NEW.estimate_version_id access
-- on tables that do not carry this column.
...
declare
  ...
  audit_estimate_version_id uuid;
begin
  if tg_table_name = 'purchase_order_items' then
    ...
    audit_estimate_version_id := nullif(to_jsonb(new)->>'estimate_version_id', '')::uuid;

    if audit_estimate_version_id is not null then
      select ev.tenant_id
        into parent_tenant_id
      from public.estimate_versions ev
      where ev.id = audit_estimate_version_id;
    end if;
