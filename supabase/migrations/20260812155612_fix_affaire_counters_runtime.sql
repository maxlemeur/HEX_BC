-- COALESCE is SQL syntax, not a catalog function. Schema-qualifying it makes
-- PostgreSQL resolve a non-existent function and breaks both the affaires list
-- and dashboard counters at runtime.

-- The table already has user-scoped RLS policies, but clean resets did not
-- restore the relation privileges required by both the invoker RPCs and the
-- favorite toggle action.
revoke all privileges
  on table public.user_favorite_projects
  from anon, PUBLIC;
...
begin
  ...
  with scoped_projects as (
    select
      ...
      project.name,
      project.client_name,
      coalesce(favorite.project_id is not null, false) as is_favorite
  ...
  searched as (
    ...
    where v_search is null
      ...
      or coalesce(project_client, '') ilike '%' || v_search || '%'
  ),
  filtered as (
    select *
    from searched
