-- Keep the public RPC contract stable while applying expensive per-project
-- aggregates only after the requested page has been selected.
...
begin
  ...
  paged_projects as materialized (
    ...
    where (
      ...
      and (
        ...
        or (
          v_sort_dir = 'desc'
          and (coalesce(lv.version_updated_at, lv.project_updated_at), lv.project_id)
            < (p_cursor_updated_at, p_cursor_project_id)
        )
        or (
          v_sort_dir = 'asc'
          and (coalesce(lv.version_updated_at, lv.project_updated_at), lv.project_id)
    ...
    order by
      case when v_sort_dir = 'asc' then coalesce(lv.version_updated_at, lv.project_updated_at) end asc,
      case when v_sort_dir = 'asc' then lv.project_id end asc,
      case when v_sort_dir = 'desc' then coalesce(lv.version_updated_at, lv.project_updated_at) end desc,
      case when v_sort_dir = 'desc' then lv.project_id end desc
    limit v_limit
  )
  select
    p.project_id,
  ...
  order by
    case when v_sort_dir = 'asc' then p.effective_updated_at end asc,
    case when v_sort_dir = 'asc' then p.project_id end asc,
    case when v_sort_dir = 'desc' then p.effective_updated_at end desc,
    case when v_sort_dir = 'desc' then p.project_id end desc;
end;
$$;
