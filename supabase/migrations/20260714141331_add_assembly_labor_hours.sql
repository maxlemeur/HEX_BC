-- Store supply quantity and labor time independently on every assembly line.

alter table public.estimate_assembly_items
  add column if not exists h_mo numeric(12, 3) not null default 0
  check (h_mo >= 0);

-- Legacy labor-only rows stored their hours in default_quantity.
update public.estimate_assembly_items
set
  h_mo = coalesce(default_quantity, 0),
  default_quantity = 1
