...
-- Consolidated from 20260307173000_est394_explanation_sources_contract_alignment.sql to keep migration versions unique.
...
begin
  if exists (
    ...
    execute $sql$
      ...
      set source_kind = coalesce(source_kind, nullif(btrim(source_type), ''))
  ...
  if exists (
    ...
    execute $sql$
      update public.estimate_explanation_sources
      set label = coalesce(label, nullif(btrim(source_label), ''))
      where label is null
    $sql$;
  end if;
  ...
  if exists (
    ...
    execute $sql$
      update public.estimate_explanation_sources
      set rank = coalesce(rank, display_rank, 0)
      where rank is null
    $sql$;
  end if;
end
