-- EST-031: split MO atelier / chantier and propagate support to estimate item RPCs.
...
where
  k_mo_atelier is null
  or k_mo_chantier is null
  or item_type = 'section';

alter table public.estimate_items
  alter column k_mo_atelier set default 1.0;

alter table public.estimate_items
  alter column k_mo_chantier set default 1.0;

alter table public.estimate_items
  drop constraint if exists estimate_items_h_mo_atelier_nonnegative_check;

alter table public.estimate_items
  add constraint estimate_items_h_mo_atelier_nonnegative_check
  check (h_mo_atelier is null or h_mo_atelier >= 0);

alter table public.estimate_items
  drop constraint if exists estimate_items_k_mo_atelier_nonnegative_check;

alter table public.estimate_items
  add constraint estimate_items_k_mo_atelier_nonnegative_check
  check (k_mo_atelier is null or k_mo_atelier >= 0);

alter table public.estimate_items
  drop constraint if exists estimate_items_h_mo_chantier_nonnegative_check;

alter table public.estimate_items
  add constraint estimate_items_h_mo_chantier_nonnegative_check
  check (h_mo_chantier is null or h_mo_chantier >= 0);

alter table public.estimate_items
  drop constraint if exists estimate_items_k_mo_chantier_nonnegative_check;

alter table public.estimate_items
  add constraint estimate_items_k_mo_chantier_nonnegative_check
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'estimate_items_labor_role_atelier_id_fkey'
      and conrelid = 'public.estimate_items'::regclass
  ) then
    alter table public.estimate_items
      add constraint estimate_items_labor_role_atelier_id_fkey
      foreign key (labor_role_atelier_id)
      references public.labor_roles(id)
      on delete set null;
  end if;
  ...
  if not exists (
    select 1
    from pg_constraint
    where conname = 'estimate_items_labor_role_chantier_id_fkey'
      and conrelid = 'public.estimate_items'::regclass
  ) then
    alter table public.estimate_items
      add constraint estimate_items_labor_role_chantier_id_fkey
      foreign key (labor_role_chantier_id)
      references public.labor_roles(id)
      on delete set null;
  end if;
end;
...
create index if not exists estimate_items_labor_role_atelier_id_idx
  on public.estimate_items (labor_role_atelier_id);

create index if not exists estimate_items_labor_role_chantier_id_idx
  on public.estimate_items (labor_role_chantier_id);
