-- Preserve the estimate supply classification on reusable assembly lines.

alter table public.estimate_assembly_items
  add column if not exists supply_type_id uuid
  references public.supply_types(id) on delete set null;

create index if not exists estimate_assembly_items_supply_type_id_idx
  on public.estimate_assembly_items (supply_type_id);

create or replace function public.validate_estimate_assembly_item_supply_type()
returns trigger
language plpgsql
set search_path = public
as $$
begin
  if new.supply_type_id is not null
    and not exists (
      select 1
      from public.supply_types st
      where st.id = new.supply_type_id
        and st.tenant_id = new.tenant_id
    )
  then
    raise exception 'supply_type_id invalide';
  end if;
...
drop trigger if exists validate_estimate_assembly_item_supply_type
  on public.estimate_assembly_items;
create trigger validate_estimate_assembly_item_supply_type
  before insert or update of tenant_id, supply_type_id
  on public.estimate_assembly_items
  for each row execute procedure public.validate_estimate_assembly_item_supply_type();
    ...
    select
      coalesce(
        sum(
          round(
            case
              when ai.cost_type = 'labor' then 0
              else
                coalesce(ai.default_quantity, 0)
                * ai.unit_cost_ht_cents
                * greatest(coalesce(ai.k_fo, 1), 0)
            end
            + coalesce(ai.h_mo, 0)
              * case
                  when lr.id is not null
                    then coalesce(lr.hourly_rate_cents, 0)
                  when ai.cost_type = 'labor'
                    then ai.unit_cost_ht_cents
                  else 0
                end
      ...
      nullif(sum(coalesce(ai.h_mo, 0)), 0) as labor_hours
