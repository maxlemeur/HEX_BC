...
where ev.id = coalesce(section_levels.version_id, line_parent_levels.version_id)
  and ev.max_section_depth is null;
...
create or replace function public.validate_estimate_version_max_section_depth()
returns trigger
language plpgsql
set search_path = public
as $$
declare
  deepest_section_level integer := 0;
  deepest_line_parent_level integer := 0;
begin
  if tg_op = 'UPDATE'
     and old.max_section_depth is not distinct from new.max_section_depth
  then
    return new;
  end if;
  ...
  with recursive section_tree as (
    ...
    from public.estimate_items child
    join section_tree on section_tree.id = child.parent_id
    where child.version_id = new.id
      and child.item_type = 'section'
  ),
  line_parent_levels as (
    select section_tree.section_level
    from public.estimate_items line
    join section_tree on section_tree.id = line.parent_id
    where line.version_id = new.id
      and line.item_type = 'line'
  )
  select
    coalesce((select max(section_level) from section_tree), 0),
    coalesce((select max(section_level) from line_parent_levels), 0)
  into
    deepest_section_level,
    deepest_line_parent_level;

  if deepest_section_level > new.max_section_depth then
    raise exception
      'max_section_depth % is incompatible: existing sections reach level %.',
      new.max_section_depth,
      deepest_section_level;
  end if;

  if deepest_line_parent_level > 0
     and deepest_line_parent_level > new.max_section_depth
  then
    raise exception
      'max_section_depth % is incompatible: existing lines are attached up to section level %.',
      new.max_section_depth,
      deepest_line_parent_level;
  end if;
...
drop trigger if exists estimate_versions_max_section_depth_guard on public.estimate_versions;

create trigger estimate_versions_max_section_depth_guard
  before update of max_section_depth
  on public.estimate_versions
  for each row execute procedure public.validate_estimate_version_max_section_depth();
...
begin
  ...
  if parent_depth > target_max_depth then
    raise exception
      'Une ligne ne peut pas etre rattachee au-dela du niveau %.',
      target_max_depth;
  end if;
...
begin
  ...
  if target_item.item_type = 'section' then
    ...
    if exists (
      ...
      raise exception 'line target_parent_id exceeds max_section_depth';
    end if;
  end if;
