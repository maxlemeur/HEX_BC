...
| TTL du verrou de brouillon | **30 minutes** | `src/lib/estimates/locks.ts:45` |
| Heartbeat du verrou | **5 minutes** | `src/hooks/useDraftLock.ts:14` |
...
**Verrou.** Expiration calculée côté serveur (`locks.ts:112-114`). Acquisition : nettoyage des verrous expirés via RPC `cleanup_expired_draft_locks` (`:446`) puis insert ; sur violation d'unicité, renouvellement silencieux si le verrou est déjà à soi (`:479-484`), sinon **409** « Cette version est déjà verrouillée par un autre utilisateur. » (`:486-493`). Hors `draft` : 403 code `READ_ONLY` (`:107-110`). Le **forçage (renouvellement ou libération) est réservé au rôle tenant `admin`** (`:302-304,368-370`). Libération sur démontage et `beforeunload`, en `keepalive` (`useDraftLock.ts:378-387,406-420`).
